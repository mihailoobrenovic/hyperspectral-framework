...
class Classifier0Cost(object):
    ...
    def setTrain(self,data_x,data_y,data_wc):
        ...
        self.train_set_wc = theano.shared(numpy.asarray(data_wc,dtype=theano.config.floatX),borrow=True)
        self.m = self.train_set_x.get_value().shape[0]
        
    def setVal(self,data_x,data_y,data_wc):
        ...
        self.val_set_wc = theano.shared(numpy.asarray(data_wc,dtype=theano.config.floatX),borrow=True)
        
    def setTest(self,data_x,data_y,data_wc):
        ...
        self.test_set_wc = theano.shared(numpy.asarray(data_wc,dtype=theano.config.floatX),borrow=True)
    ...
    def train(self,n_epoha=100,mini_batch_size=100,learning_rate=0.1): 
        self.n_epoha = n_epoha
        x = T.dmatrix('x')
        lab = T.dmatrix('lab')
        index = T.lscalar('index')
        wc = T.dvector('wc')
        izlaz = self.sigmoid(x,self.W_class,self.b_class)
        L = - wc * T.sum(lab * T.log(izlaz) + (1 - lab) * T.log(1 - izlaz), axis=1)
        cost = T.mean(L)
        ...
        funkcija_treniranja = theano.function([index],[cost],updates=updates,givens={ x:self.train_set_x[index:index+mini_batch_size,:], lab:self.train_set_y[index:index+mini_batch_size,:], wc:self.train_set_wc[index:index+mini_batch_size]} )
        funkcija_validacije = theano.function([],[cost],givens={ x:self.val_set_x, lab:self.val_set_y, wc:self.val_set_wc} )
    ...
    def plot_cost_over_time(self):
        ...
        n_epoha = self.n_epoha
        for e in xrange(n_epoha):
            epohe_x.append(e)
            
        self.plot_costovi(epohe_x,self.costovi,"train","cost1")
        self.plot_costovi(epohe_x,self.validacija,"validation","cost1")
        plt.legend()
