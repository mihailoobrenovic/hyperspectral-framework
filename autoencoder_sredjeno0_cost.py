...
class AutoEncoder0Cost(object):
    ...
    def __init__(self,load_data=True,n_visible=200,n_hidden=100,W=None,bhid=None,bvis=None,dataset='Indian_pines_corrected.mat',labels='Indian_pines_gt.mat',data_name="indian_pines_corrected",label_name="indian_pines_gt"):
        if load_data:            
            self.train_set_x,self.train_set_y,self.train_set_wc,self.val_set_x,self.val_set_y,self.val_set_wc,self.test_set_x,self.test_set_y,self.test_set_wc = self.load_hyperspectral0(dataset,labels,data_name,label_name)
...
#                exit()
   ...
   # Racuna prvi skriveni sloj autoenkodera     
    ...
    def load_hyperspectral0(self,dataset,labels,data_name='indian_pines_corrected',label_name='indian_pines_gt'):
        ...
        # prebroj po klasama           
        count_class = sum(mat_lab)
        percent = count_class/total
        weight_class = 1/(percent*max_class)
        
        weight_elemwise = numpy.zeros([total])
        for i in range(0,total):
            weight_elemwise[i] = weight_class[numpy.argmax(mat_lab[i,:])]
        ...
        train_set_wc = weight_elemwise[:int(total*0.6)]
        val_set_wc = weight_elemwise[int(total*0.6):int(total*0.8)]
        test_set_wc  =weight_elemwise[int(total*0.8):]
        ...
        return (theano.shared(numpy.asarray(train_set_x,dtype=theano.config.floatX),borrow=True),
                ...
                theano.shared(numpy.asarray(train_set_wc,dtype=theano.config.floatX),borrow=True),
                ...
                theano.shared(numpy.asarray(val_set_wc,dtype=theano.config.floatX),borrow=True),
                ...
                theano.shared(numpy.asarray(test_set_wc,dtype=theano.config.floatX),borrow=True))
